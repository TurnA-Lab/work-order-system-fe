...
export default class Console {
  ...
  // 打印响应时间
  public static getLoadTime(infoStyle: InfoStyleConfig = this.defaultStyle): void {

    console.log(
      `%c Load time: ${(Math.round(performance.now() * 100) / 100 / 1000).toFixed(2)}s `,
  ...
  public static getJoyUser(infoStyle: InfoStyleConfig = this.defaultStyle) {
    ...
    console.log(
      `%c You visited here ${hit} time${hit > 1 ? "s" : ""} since ${formatedTime.getFullYear()}.${formatedTime.getMonth()}.${formatedTime.getDate()} ${formatedTime.getHours()}:${formatedTime.getMinutes()}. `,
      `color: ${infoStyle.color}; padding: 2px 0px;`
    );
